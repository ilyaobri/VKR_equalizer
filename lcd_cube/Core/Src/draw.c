...
void frame_init(frame_t *frame, u16 *canvas, u32 width, u32 height) {
    ...
    frame->width = width;
    frame->height = height;
    frame->total_points = width * height;
}

void frame_draw_rect(frame_t *frame, color_t color, u32 x0, u32 y0, u32 x1, u32 y1) {
    for (u32 y = y0; y <= y1; y++) {
        u32 offset = frame_calc_offset(frame, y);
        for (u32 x = x0; x <= x1; x++) {
            u32 index = offset + x;
            frame_set_by_index(frame, index, color);
        }
    }
}

void frame_fill(frame_t *frame, color_t color) {
    frame_draw_rect(frame, color, 0, 0, frame->width - 1, frame->height - 1);
}

uc8 asc2_1206[FONT_SUPPORTED_CHARACTERS][FONT_SIZE_12] = {
...
uc8 asc2_1608[FONT_SUPPORTED_CHARACTERS][FONT_SIZE_16] = {
...
void frame_draw_char(frame_t *frame, color_t color, u32 x, u32 y, font_size_t size, char ch) {
    assert(size == FONT_SIZE_16 || size == FONT_SIZE_12);

    u8 temp = 0;

    u8 num = (u8) ch - ' ';  // table start from space character

    for (u32 pos = 0; pos < size; pos++) {
        u32 offset = frame_calc_offset(frame, y + pos);

        if (size == FONT_SIZE_12) {
            temp = asc2_1206[num][pos];
        } else if (size == FONT_SIZE_16) {
            temp = asc2_1608[num][pos];
        }

        for (u32 bit = 0; bit < size / 2; bit++) {
            if (xbit_u32(temp, bit))
                frame_set_by_index(frame, offset + x + bit, color);
        }
    }
}
...
void frame_draw_string(frame_t *frame, color_t color, u32 x, u32 y, font_size_t size, string str) {
    ...
    for (u32 k = 0; k < length; k++) {
        u32 xc = x + size / 2 * k;
        frame_draw_char(frame, color, xc, y, size, str[k]);
    }
}
...
void frame_printf_string(frame_t *frame, color_t color, u32 x, u32 y, font_size_t size, string format, ...) {
    ...
    u32 length = vsprintf(NULL, format, args);
    assert(length > 0);
    ...
    while ((token = strsep(&str, "\n"))) {
        u32 yc = y + size * line++;
        frame_draw_string(frame, color, x, yc, size, token);
    }
